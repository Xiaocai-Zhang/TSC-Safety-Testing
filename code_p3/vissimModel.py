...
class Vissim_Server:
    ...
    def ScDurationExecution(self,CurPhase):
        veh_num_li_all = []
        for i in range(8):
    ...
    def ScDurationExecutionAmberRed(self, CurPhase):
        veh_num_li_all = []
        for i in range(3):
    ...
    def GetTrafficSignalOup(self):
        all_veh_attributes = self.Vissim.Net.Vehicles.GetMultipleAttributes(
            ('Lane\Link', 'Lane\Index', 'No', 'Speed', 'Acceleration ', 'Pos', 'VehType'))
        df_attributes = pd.DataFrame(all_veh_attributes,
                                     columns=['Lane\Link', 'Lane\Index', 'No', 'Speed', 'Acceleration', 'Pos',
                                              'VehType'])
        df_attributes = df_attributes[df_attributes['VehType'].isin(['100', '200', '300', '630', '640', '650'])]
        df_attributes = df_attributes.dropna(subset=['Pos'])

        action = config.test_agent.choose_action(df_attributes)
        return action
    ...
    def ActionExecution(self, action1, CurPhase):
        ...
        t = 0
        while True:
            veh_num = self.ScDurationExecution(CurPhase)
            t = t + 8
            nexPhase = self.GetTrafficSignalOup()
            if nexPhase != CurPhase:
                t = 0
                veh_num = self.ScDurationExecutionAmberRed(CurPhase)
                veh_num_li = veh_num_li + veh_num
                CurPhase = nexPhase
            else:
                if t > 50: nexPhase = self.InferNextPhase(CurPhase)
                CurPhase = nexPhase
            veh_num_li = veh_num_li + veh_num
